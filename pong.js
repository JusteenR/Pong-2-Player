...
class Game {
  constructor(canvas){
    ...
    setInterval ((e)=> {this.player(this.p1, this.canvas, up, down);}, 5);
    setInterval ((e)=> {this.player(this.cpu, this.canvas, up2, down2);}, 5);

  }


  player(p1, canvas, up, down){

    if (down === true){
      p1.pos.y += 1;
      if (p1.pos.y > canvas.height){
        p1.pos.y = canvas.height;
      }
    }
    ...
    if (up === true){
     ...
     //requestAnimationFrame(()=>this.player()); Used setInterval instead
